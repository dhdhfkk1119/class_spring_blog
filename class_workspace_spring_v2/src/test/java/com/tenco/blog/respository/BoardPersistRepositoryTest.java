...
public class BoardPersistRepositoryTest {
    ...
    public void deleteById_test(){
        Long id = 1L;
        Board target = boardPersistRepository.findById(id);
        Assertions.assertThat(target).isNotNull();

        boardPersistRepository.deleteById(id);
        List<Board> afterBoard = boardPersistRepository.findAll();
        Assertions.assertThat(afterBoard.size()).isEqualTo(3);
        Assertions.assertThat(afterBoard.get(2)).isNotNull();
    }
}
