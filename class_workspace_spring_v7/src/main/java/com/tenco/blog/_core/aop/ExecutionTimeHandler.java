...
public class ExecutionTimeHandler {
        /*
        * 1. 요청부터 응답까지의 실행시간을 측정하여 로그에 기록하는 Advice로 지정 함
        * 2. JoinPoint 지정 (특정 시점 정의) Around : 메서드의 실정 전과 후에 동작하라 명시하라
        * 3. PointCut 어떤 메서드가 실행될때 (Advice) 가 동작할지 지정 함
        * (표현식 지정 가능 - execution(*com.tenco.blog..*(..)))
        * 4. 어떤일을 수행하는지 구체적으로 명시
        * 
        * */
    ...
    public Object logExecutionTime(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        // 예시 : 학교에 행사를 진행하는 담당자 , 행자 진행 전 준비 , 끝나고 마무리를 챙김
        // 이제 행사 시작 - 신호를 주면 실제 행사(메서드가) 진행 됨
        // 1. 요청 시작 시간을 기록
        long startTime = System.currentTimeMillis();
        // 2. 원래 진행 하고자 하는 메서드를 진행해라고 신호
        Object result;
        try{
